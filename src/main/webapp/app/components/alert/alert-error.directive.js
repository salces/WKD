(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('wkdProjectApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-wkdProjectApp-error');
                var entityKey = httpResponse.headers('X-wkdProjectApp-params');
                if (errorHeader) {
